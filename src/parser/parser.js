...
function peg$parse(input, options) {
  ...
  var peg$f2 = function (definitions) {
    ...
    defs.forEach((d) => {
      const index = defs.indexOf(d);
      const original = d;
      const html = `<dd>${cleanedDefs[index]}</dd>`;
  ...
  var peg$f5 = function (t, underline) {
    ...
    const originalText = textArr.map((t) => (typeof t === 'object' ? t.original : t)).join('');
    const htmlText = textArr.map((t) => (typeof t === 'object' ? t.html : t)).join('');
    const original = originalText + '\n' + underlineArr.join('');
    const html = `<h${level}>${htmlText}</h${level}>`;
  ...
  var peg$f6 = function (hashes, headerText) {
    ...
    html += `</h${headerLevel}>`;
    if (headerLevel > 6) {
      html = html.replace(`<h${headerLevel}>`, `<p>${hashArr.join('')} `).replace(`</h${headerLevel}>`, '</p>');
      return { type: 'paragraph', original, html };
    }
    return { type: 'header', headerLevel, original, html };
  };
  var peg$f7 = function (blockquotes) {
    const original = blockquotes.map((b) => b.original).join('');
    let html = '<blockquote>';
  ...
  var peg$f10 = function (spaces, items) {
    ...
    const original = items.map((i) => i.original).join('');
  ...
  var peg$f11 = function (spaces, item) {
    return item;
  };
  var peg$f12 = function (spaces, t) {
    ...
    const objects = textArr.filter((t) => typeof t === 'object');
  ...
  var peg$f15 = function (t) {
    ...
    const filteredText = text.filter((t) => (typeof t === 'object' && t.type !== 'html') || typeof t === 'string');
    let original = '';
    let html = '';
  ...
  var peg$f30 = function (altText, url) {
    ...
    const sourceUrl = urlSplit[0];
    const title = urlSplit.length > 1 ? urlSplit[1].replace(/"/g, '') : '';
    const imageTitle = title !== '' ? ` title="${title}"` : '';
    const html = `<img src="${sourceUrl}" alt="${altText}"${imageTitle}/>`;
    return { type: 'image', original, html };
  };
  var peg$f31 = function (linkText, url) {
    const original = `[${linkText}](${url})`;
    const urlSplit = url.split(/ (.+)/).filter((part) => part.trim() !== '');
    const sourceUrl = urlSplit[0];
    const title = urlSplit.length > 1 ? urlSplit[1].replace(/"/g, '') : '';
    const linkTitle = title !== '' ? ` title="${title}"` : '';
    const html = `<a href="${sourceUrl}"${linkTitle}>${linkText}</a>`;
